...
class MyMayaKubeSpawner(KubeSpawner):
    def __init__(self, *args, **kwargs):
        ...
        self.pvc_mayastor_name = self._expand_user_properties(self.pvc_mayastor_name_template)
    ...
    pvc_mayastor_name_template = Unicode(
        'mayastor-claim-{username}',
        config=True,
        help="Template to use to form the name of mayastor's pvc.",
    )
    ...
    async def _start(self):
        ...
        if self.storage_pvc_ensure:
            ...
            pvc_mayastor = self.get_pvc_mayastor_manifest()
            ...
            await exponential_backoff(
                partial(
                    self._make_create_pvc_request, pvc_mayastor, self.k8s_api_request_timeout
                ),
                f'Could not create PVC {self.pvc_mayastor_name}',
                # Each req should be given k8s_api_request_timeout seconds.
                timeout=self.k8s_api_request_retry_timeout,
            )
    ...
    def get_pvc_mayastor_manifest(self):
        """
        Make a pvc manifest that will spawn current user's pvc.
        """
        labels = self._build_common_labels(self._expand_all(self.storage_extra_labels))
    ...
    async def delete_forever(self):
        ...
        await exponential_backoff(
            partial(
                self._make_delete_pvc_request,
                self.pvc_mayastor_name,
                self.k8s_api_request_timeout,
            ),
            f'Could not delete pvc {self.pvc_mayastor_name}',
            timeout=self.k8s_api_request_retry_timeout,
        )


c.JupyterHub.spawner_class = MyMayaKubeSpawner

## Instructors vs students

c.Spawner.cmd = ['start.sh','jupyterhub-singleuser','--allow-root']
c.MyMayaKubeSpawner.args = ['--allow-root']
