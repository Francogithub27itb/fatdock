...
class MyKubeSpawner(KubeSpawner):
    def __init__(self, *args, **kwargs):
        ...
        self.pvc_longhorn_name = self._expand_user_properties(self.pvc_longhorn_name_template)
    ...
    pvc_longhorn_name_template = Unicode(
        'longhorn-claim-{username}',
        config=True,
        help="Template to use to form the name of longhorn's pvc.",
    )
    ...
    async def _start(self):
        ...
        if self.storage_pvc_ensure:
            ...
            pvc_longhorn = self.get_pvc_longhorn_manifest()
            ...
            await exponential_backoff(
                partial(
                    self._make_create_pvc_request, pvc_longhorn, self.k8s_api_request_timeout
                ),
                f'Could not create PVC {self.pvc_longhorn_name}',
                # Each req should be given k8s_api_request_timeout seconds.
                timeout=self.k8s_api_request_retry_timeout,
            )
    ...
    def get_pvc_longhorn_manifest(self):
        """
        Make a pvc manifest that will spawn current user's pvc.
        """
        labels = self._build_common_labels(self._expand_all(self.storage_extra_labels))
    ...
    async def delete_forever(self):
        ...
        await exponential_backoff(
            partial(
                self._make_delete_pvc_request,
                self.pvc_longhorn_name,
                self.k8s_api_request_timeout,
            ),
            f'Could not delete pvc {self.pvc_longhorn_name}',
            timeout=self.k8s_api_request_retry_timeout,
        )
...
c.MyKubeSpawner.args = ['--allow-root']
